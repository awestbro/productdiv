...
function toStyleMultilineString(styleString: string) {
    const lines = styleString.split(';').map((x) => x.trim()).filter((x) => x !== '');
    return lines.reduce((acc, l, i) => {
        const [style, value] = l.split(':').map((x) => x.trim()).filter((x) => x !== '');
        if (i === 0) {
            return `${style}: ${value};`;
        }
        return `${acc}\n${style}: ${value};`;
...
export function AttributeEditor(props: LeftNavProps) {
    ...
    const styles = element.attributes.getNamedItem('style') ? element.attributes.getNamedItem('style').value : '';
    const [styleString, setStyleString] = React.useState<string>(toStyleMultilineString(styles));

    function onAttributeNameChange(oldName: string, _newName: string, value: string) {
            ...
            <div className="container mt-2">
                ...
                {Object.keys(attributes).map((a, i) => (
                    <div key={i} className="row align-items-center">
                        <div className="col-sm-4 px-0 text-truncate">
                            <input onBlur={(e) => onAttributeNameChange(a, e.target.value, attributes[a])} className="form-control text-light" type="text" defaultValue={a} />
                        </div>
                        <div className="col-sm-7 px-0">
                            <input className="form-control text-light" type="text" value={attributes[a]} onChange={(e) => onAttributeValueChange(a, e.target.value)} />
                        </div>
                ...
                <div className="row mt-3">
                    <div className="col-sm-12">
                        <label className="mb-2" htmlFor="style">Style</label>
                        <CodeMirror
                            value={styleString}
                            options={{
                                mode: 'css',
