...
export function drawMarginAndPadding(
  ...
  ctx.rect(
    ...
    width - numPaddingLeft - numBorderLeft - numPaddingRight - numBorderRight,
    height - numBorderTop - numPaddingTop - numBorderBottom - numPaddingBottom
  );
...
export function drawPlacementHover(
  mouseX: number,
  mouseY: number,
  ctx: CanvasRenderingContext2D,
  element: Element,
...
export function drawPlacementType(
  ctx: CanvasRenderingContext2D,
  element: Element,
  ...
  if (placementType === PlacementType.OUTER_PREPEND) {
    ...
    if (placementType === PlacementType.INNER_PREPEND) {
      ...
      ctx.fillStyle = hoverBorderActiveColor;
      ctx.fillRect(
        innerLeft + (innerRight - innerLeft) / 2 - borderSize,
        innerTop - borderSize,
        (innerRight - innerLeft) / 2,
        innerBottom - innerTop
      );
