...
export function createComponentTree(doc: Document, rootElement: Element, treeViewIgnoreQuerySelectors: string[]): NodeTreeMatch[] {
    ...
    return removeIgnoredTreeMatches(doc, matchNodes, treeViewIgnoreQuerySelectors);
}

function removeIgnoredTreeMatches(doc: Document, matches: NodeTreeMatch[], treeViewIgnoreQuerySelectors: string[]): NodeTreeMatch[] {
    const ignoreElements: Element[] = treeViewIgnoreQuerySelectors.reduce((acc, selector) => {
...
function matchNodesToComponentTree(nodes: Node[]): NodeTreeMatch[] {
    return nodes.map((node) => {
        ...
        } else if (node.nodeType === ChildNodeType.Text && node.textContent.match(/^\s+$/)) {
...
export function nodePrettyName(node: Node): string {
    if (node.nodeType === ChildNodeType.Element) {
        ...
        if (n.classList.length > 0) {
            const classString = domTokenListToArray(n.classList).reduce((acc: string, curr: string) => `${acc}.${curr}`, '');
...
export function getParentMatch(tree: NodeTreeMatch[], match: NodeTreeMatch): NodeTreeMatch {
...
export function getTreeMatchFromElement(tree: NodeTreeMatch[], element: Node): NodeTreeMatch {
    function find(m: NodeTreeMatch): boolean {
        if (m.node == element) {
            result = m;
            return true;
